...
def plot_rocs_wrapper(figname_prefix, LRs_varS2, H_2plot, true_s_2plot, panel_labs, sim_init, LL_init, which_s: str):
    roc_figname = f'{figname_prefix}_ROCs_forPoster.png'
    # three cols of ROCs
    roc_fig, roc_ax = plt.subplots(nrows=1, ncols=len(H_2plot), sharex=True, sharey=True,
                                   figsize=(2.5 * len(H_2plot), 3))
    # plot
    for i, h in enumerate(H_2plot):
        roc_ax[i] = plot_ROCs(roc_ax[i], LRs_varS2, H=h, true_s2s=true_s_2plot, which_s=which_s)
        roc_ax[i].set_title(f'{panel_labs[i]}$h={h}$', loc='center', fontsize="medium", fontweight='bold')
        # adjust border
        roc_ax[i].tick_params(axis='both', labelsize="x-small", width=0.5, direction='inout')
        for side in roc_ax[i].spines.keys():
            roc_ax[i].spines[side].set_linewidth(0.5)
    ...
    roc_ax[0].legend(handles, labels, loc="lower right",  # loc="best", bbox_to_anchor=(0.5, 0), borderaxespad=6,
    ...
    if len(H_2plot) < 3:
        roc_fig.suptitle(f'Simulation initialized with {sim_init} condition.\n'
                         f'Likelihood computed under {LL_init} initial state.', fontsize='small')  #
    else:
        roc_fig.suptitle(f'Simulation initialized with {sim_init} condition. '
                         f'Likelihood computed under {LL_init} initial state.', fontsize='medium')  #
    plt.tight_layout()  # h_pad=1.5
    roc_fig.savefig(roc_figname, dpi=500, transparent=True)
    print(roc_figname)
...
def main():
    ...
    for i, llInit in enumerate(llInits):
        plot_rocs_wrapper(f'{figname_prefix}_ll{llInit}_0-1H', metaLRs['s2'][llInit], H_2plot[0], true_s_2plot[0],
                          panel_labs=['', '', ''], sim_init=simInit, LL_init=llInit, which_s='s2')  # , 'D'
        # clean the slate
        plt.clf()

        plot_rocs_wrapper(f'{figname_prefix}_ll{llInit}_5-InfH', metaLRs['s1'][llInit], H_2plot[1], true_s_2plot[1],
                          panel_labs=['', ''], sim_init=simInit, LL_init=llInit, which_s='s1')  # , 'D', 'C'
        # clean the slate
        plt.clf()
