...
plotManChrs <- function(dt, left, right, statname, colorstat,
  ...
  p <- ggplot(dt)
  chr = unique(dt$Chr)[1]
  # p <- p  + geom_point (aes(x=physPos, y=stat, color=colorscale), size=3) + xlab(paste('Position on chromosome', chr))
  p <- p  + geom_point_rast(aes(x=as.numeric(physPos), y=as.numeric(stat), color=as.numeric(colorscale)), size=3, raster.dpi = 600) + xlab(paste('Position on chromosome', chr))
...
plotFancyGenes <- function(genes, noRNA=FALSE){
  ...
  # hack the position
  Names[Names$name == "MAP3K19",]$layer <- 1.2
  ###start plotting
  ...
  p <- p + geom_text(data=Names, aes(y=-10*layer+2, x=pos, label=name), size=4,
                           family='Helvetica', fontface='bold.italic', color='black',
                           segment.size=0, direction='y', ylim=c(NA, -6))
...
plotPeak <- function(ch, inputname, left, right, outputname, statname, uncorrected_cutoff=0.05){
  ...
  DT$chi2_p <- as.numeric(DT$chi2_p)
  ...
  if(nrow(genes)>0){
    ...
    pg <- pg + scale_x_continuous(name=paste('Position on chromosome', ch, '(Mbp)'), labels=function(x){return(as.character(x/1e6))}, limits=c(left, right)) + 
    ...
    pg <- pg +  #lambda x: as.character(x/1e6)
