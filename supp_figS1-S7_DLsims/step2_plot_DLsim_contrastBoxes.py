...
def load_pkl_MLEs(s2_pkl_name, s1_pkl_name):
    ...
    for i, pkl_bash_name in enumerate([s2_pkl_name, s1_pkl_name]):
        ...
        # hack it
        tmp_fields = pkl_bash_name.split('\\')
        pkl_output = subprocess.check_output(f"ls {''.join(tmp_fields)}", shell=True).decode()
        # print(pkl_output)
...
def main():
    ...
    for i, llInit in enumerate(llInits):
        ...
        # not clean, but whatevs
        plotIt = ((figname_prefix.startswith("simInitSV") and (llInit == "Unif")) or
                    (figname_prefix.startswith("simInitFreq.01") and (llInit == "Unif")) or
                    (figname_prefix.startswith("simInitFreq.01") and (llInit == "Freq")))
        if (not plotIt):
            break
        ...
        fig.suptitle(f'Simulation initialized w. {_express_init(simInit)}.  Likelihood computed w. {_express_init(llInit)}.',
                     fontsize='medium', ha='center', va='top', y=0.95) #
        plt.tight_layout()
        # fig.savefig(f'{figname_prefix}_LLinit{llInit}_MLE_twoBoxes.png', dpi=350, transparent=True)
        fig.savefig(f'{figname_prefix}_LLinit{llInit}_MLE_twoBoxes.pdf', dpi=350, transparent=True)
