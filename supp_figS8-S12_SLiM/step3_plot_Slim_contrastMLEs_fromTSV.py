...
def main():
    ...
    box_fig, (ax1, ax2) = plt.subplots(ncols=2, figsize=(7, 3.5), sharey=True,
                                       gridspec_kw={'width_ratios': (0.5, 0.27)})

    if 'selected' in s2_scoreFile:
        ax1 = sns.boxplot(x='s2', y='shat', hue=s2_scores.h.astype(str), data=s2_scores, palette=H_colors,
                          fliersize=0.5, orient="v", linewidth=0.5, ax=ax1)
        ax1.set_title(r'A) Inferred $~\widehat{s}_{AA}$', loc='left', fontsize='medium')
        # annotate outliers
        ...
        ax1.set_title(r'A) Inferred $~|\widehat{s}_{AA}|$ of top LR sites', loc='left', fontsize='medium')
        # annotate outliers
    ...
    if 'selected' in s1_scoreFile:
        ...
        ax2.set_title(r'B) Inferred $~\widehat{s}_{Aa}$', loc='left', fontsize='medium')
        # annotate outliers
        ...
        ax2.set_title(r'B) Inferred $~|\widehat{s}_{Aa}|$ of top LR sites', loc='left', fontsize='medium')
        # annotate outliers
    ...
    # hack
    ax2.set_xticklabels(labels=['0.0'] + [f'{s1:.3f}' for s1 in s1_list_2plot[1:]])
    ...
    # figname1 = f'{wdir}{fig_prefix}_200kb_t9x500gen_n40_{on_off_grid}Grid_MLEs_contrastBoxes.png'
    figname1 = f'{wdir}{fig_prefix}_200kb_t9x500gen_n40_{on_off_grid}Grid_MLEs_contrastBoxes.pdf'
    # figname = f'{wdir}DL-1D_200kb_t9n40_{on_off_grid}Grid-maxLLR_loc_violinplots.png' #
