...
plotManChrs <- function(dt, left, right, statname, colorstat,
  ...
  chr = unique(dt$Chr)[1]
  if(colorstat != FALSE){
    p <- p  + geom_point_rast(aes(x=physPos, y=stat, color=colorscale), size=1.5, raster.dpi = 600)
    ...
    p <- p  + geom_point_rast(aes(x=physPos, y=stat), color='#666666', size=1.5, raster.dpi = 600)
              scale_x_continuous(name=paste('Positions on chromosome', ch, '(Mbp)'), labels=function(x){return(as.character(x/1e6))}, limits=c(left, right))
  }
...
plotSingleChrom <- function(ch, inputname, outputname, statname){
  ...
  ### format
  DT$physPos <- as.numeric(DT$physPos)/1000000
  ...
  p1 <- p1 + xlab(paste('Position on chromosome', ch) ) +
