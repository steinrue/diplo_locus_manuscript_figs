...
ALLEN_VERSION = "v54.1"
ALLEN_ROOT = (f'{ALLEN_VERSION}_1240K/{ALLEN_VERSION}_1240K_public')
INDIVIDUALS_FILE = pathlib.Path('extracted', f'UK_{ALLEN_VERSION}_all_with_duplicates.table')
...
def extractTimeSeries(c):
    ...
    assert c in chromHist.keys(), chromHist.keys()
    ...
    # run it
    result = subprocess.run([stratCmd], shell=True, capture_output=True, text=True)
    print(result.stdout)
    ...
    # run it
    result = subprocess.run([haploCmd], shell=True, capture_output=True, text=True)
    print(result.stdout)
